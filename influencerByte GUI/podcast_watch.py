...
GUI for the podcast platform advertising for a smart watch
"""
...
dynamic = True
count = 0
#load the pre-scraped data for watch into a dataframe
data = data = pd.read_excel('patreon_cleaned.xls', sheet_name='Smart Watch')
...
def go_click():
    print("\nRecommending you an influencer to market your smart watch...\n")
    global dynamic
    global count
    global data
    ...
    if (dynamic == True):
        ...
        exec(open('patreon_watch.py').read())
...
#Tkinter Code
window = Tk()
window.title("PODCAST - WATCH")
window.geometry('500x200')
header = Label(window, text="SMART WATCH", font=("Britannic Bold", 18))
