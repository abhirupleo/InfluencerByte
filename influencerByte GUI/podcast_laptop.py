...
GUI for the podcast platform advertising for a laptop.
"""
...
dynamic = True
count = 0
#load the pre-scraped data for laptop into a dataframe
data = pd.read_excel('patreon_cleaned.xls', sheet_name='Laptop')
...
def go_click():
    ...
    global count
    global data
    #if user selected dynamic scraping, run the patreon_laptop file to scrape the data
    if (dynamic == True):
        print('SCRAPING DATA... please wait')
        import patreon_laptop
        exec(open('patreon_laptop.py').read())
